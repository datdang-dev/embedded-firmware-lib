...
Status Aes128Algorithm::encrypt(std::span<const uint8_t> input,
...
Status Aes128Algorithm::decrypt(std::span<const uint8_t> input,
                                std::span<uint8_t> output) {
  // Validate buffers
  if (input.empty() || output.size() < input.size()) {
    return Status(types::StatusCode::ERR_INSUFFICIENT_BUFFER);
...
void Aes128Algorithm::encryptBlock(const uint8_t *input, uint8_t *output,
...
void Aes128Algorithm::decryptBlock(const uint8_t *input, uint8_t *output,
                                   const uint8_t * /*key*/) const {
