 ...
 * In production, use a real AES implementation (e.g., mbedTLS, hardware
 * crypto).
 *
...
inline constexpr uint8_t kMockEncryptKey = 0x00U; //
...
Status Aes256Algorithm::encrypt(std::span<const uint8_t> input,
...
Status Aes256Algorithm::decrypt(std::span<const uint8_t> input,
                                std::span<uint8_t> output) {
  // Validate buffers
  if (input.empty() || output.size() < input.size()) {
    return Status(types::StatusCode::ERR_INSUFFICIENT_BUFFER);
...
void Aes256Algorithm::encryptBlock(const uint8_t *input, uint8_t *output,
...
void Aes256Algorithm::decryptBlock(const uint8_t *input, uint8_t *output,
                                   const uint8_t * /*key*/) const {
