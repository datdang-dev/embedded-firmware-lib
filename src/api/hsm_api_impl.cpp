...
bool HsmApiImpl::isReady() const { return isInitialized_; }
...
Status HsmApiImpl::encrypt(types::SessionId sessionId, uint8_t keySlotId,
  ...
  if ((info.permissions &
       static_cast<uint8_t>(types::KeyPermission::ENCRYPT)) == 0U) {
...
Status HsmApiImpl::decrypt(types::SessionId sessionId, uint8_t keySlotId,
                           types::Algorithm algorithm,
                           std::span<const uint8_t> input,
                           std::span<uint8_t> output) {
  if (!isInitialized_) {
  ...
  if ((info.permissions &
       static_cast<uint8_t>(types::KeyPermission::DECRYPT)) == 0U) {
    return Status(types::StatusCode::ERR_AUTH_FAILED);
  }
...
Status HsmApiImpl::importKey(uint8_t keySlotId, types::Algorithm algorithm,
                             std::span<const uint8_t> keyData,
                             uint8_t permissions) {
  if (!isInitialized_) {
    return Status(types::StatusCode::ERR_NOT_INITIALIZED);
  }
  return keystoreService_->importKey(keySlotId, algorithm, keyData,
                                     permissions);
}
...
Status HsmApiImpl::getKeySlotInfo(uint8_t keySlotId, types::KeySlotInfo &info) {
  if (!isInitialized_) {
    return Status(types::StatusCode::ERR_NOT_INITIALIZED);
  }
...
const char *HsmApiImpl::getVersion() const { return kVersionString; }
